...
use Google\AdsApi\Dfp\v201805\ReportJobStatus;
...
class ReportDownloader
{
    ...
    public function __construct(
        ...
        $this->logger = $reportService->getAdsSession()->getReportDownloaderLogger();
        $this->reportService = $reportService;
        $this->reportJobId = $reportJobId;
        $this->pollTimeSeconds = $pollTimeSeconds === null ? self::DEFAULT_POLL_SECONDS : $pollTimeSeconds;
    ...
    {
        $reportJobStatus = $this->reportService->getReportJobStatus($this->reportJobId);
    ...
    public function downloadReport($exportFormat, $filePath = null)
    {
        ...
        $proxy = $this->reportService->getAdsSession()->getConnectionSettings()->getProxyUrl();
        if (!empty($proxy)) {
    ...
    private function getFormattedUserAgent()
    {
        ...
        return $session->getAdsHeaderFormatter()->formatApplicationNameForGuzzleHeader(
