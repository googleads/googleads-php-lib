...
use Google\AdsApi\AdWords\v201710\rm\FirstPartyUploadMetadata;
...
class UploadOfflineData
{
    ...
    // third party data.
    const STORE_SALES_UPLOAD_COMMON_METADATA_TYPE = FirstPartyUploadMetadata::class;
    ...
    public static function runExample(
        ...
        $offlineDataUploadService = $adWordsServices->get($session, OfflineDataUploadService::class);
        ...
        $reflectionClass = new \ReflectionClass($storeSalesUploadCommonMetadataType);
        ...
        if ($storeSalesUploadCommonMetadataType === FirstPartyUploadMetadata::class) {
            $offlineDataUpload->setUploadType(
                OfflineDataUploadType::STORE_SALES_UPLOAD_FIRST_PARTY
            );
        } elseif ($storeSalesUploadCommonMetadataType === ThirdPartyUploadMetadata::class) {
            $offlineDataUpload->setUploadType(
        ...
        printf(
            "Uploaded offline data with external upload ID %d and upload status %s\n",
        ...
        foreach ($offlineDataUpload->getPartialDataErrors() as $partialDataError) {
            if ($partialDataError !== null) {
                printf(
                    "Found a partial error for offline data #%d with error string: %s\n",
                    $i,
