 ...
 *
 * NOTE: Since this example loads all line items into memory, your PHP
 *       memory_limit may need to be raised for this example to work properly.
 *
 * The Line_Item PQL table schema can be found here:
 * https://developers.google.com/doubleclick-publishers/docs/reference/v201308/PublisherQueryLanguageService#Line_Item
 *
...
try {
  ...
  $pqlService = $user->GetService('PublisherQueryLanguageService', 'v201308');

  // Statement parts to help build a statement to select all line items.
  $pqlTemplate = 'SELECT Id, Name, Status FROM Line_Item ORDER BY Id ASC LIMIT '
      . '%d OFFSET %d';
  ...
  $filePath = sprintf("%s/Line-Items-%s.csv", sys_get_temp_dir(), uniqid());
  ob_start();
  $fp = fopen($filePath, 'w');
