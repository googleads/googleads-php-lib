<?php
/**
 * This example gets all custom fields that can be applied to line items. To
 * create custom fields, run CreateCustomFields.php.
 *
 * PHP version 5
 *
...
try {
  ...
  $customFieldService = $user->GetService('CustomFieldService', 'v201605');

  // Create a statement to select only custom fields that can be applied to
  // line items.
  $statementBuilder = new StatementBuilder();
  $statementBuilder->Where('entityType = :entityType')
      ->OrderBy('id ASC')
      ->Limit(StatementBuilder::SUGGESTED_PAGE_LIMIT)
      ->WithBindVariableValue('entityType', 'LINE_ITEM');
