<?php
/**
 * This example creates programmatic proposals. Your network must have Sales
...
// Set the ID of the buyer. This can be obtained through the Programmatic_Buyer
// PQL table.
$buyerId = 'INSERT_BUYER_ID_HERE';
...
try {
  ...
  $proposalService = $user->GetService('ProposalService', 'v201705');
  $networkService = $user->GetService('NetworkService', 'v201705');

  // Create a proposal.
  $proposal = new Proposal();
  $proposal->name = 'Programmatic proposal #' . uniqid();

  // Set the required Marketplace information.
  $marketplaceInfo = new ProposalMarketplaceInfo();
  $marketplaceInfo->buyerAccountId = $buyerId;
  $proposal->marketplaceInfo = $marketplaceInfo;
  ...
  foreach ($proposals as $createdProposal) {
    printf("A programmatic proposal with ID %d and name '%s' was created.\n",
