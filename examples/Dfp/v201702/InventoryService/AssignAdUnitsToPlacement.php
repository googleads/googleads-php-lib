 ...
 * placement. The ad units will override the current placement settings, not
 * append to them. To determine which ad units exist, run GetAllAdUnits.php. To
...
// Set the ad unit ID to set on the placement.
...
try {
  ...
  $inventoryService = $user->GetService('InventoryService', 'v201702');
  ...
  do {
    ...
    if (isset($page->results)) {
      ...
      foreach ($page->results as $adUnit) {
        printf("%d) Ad unit with ID %d, and name '%s' will be assigned to "
  ...
  printf("Number of ad units to be assigned to the placement: %d\n",
      $totalResultSetSize);
  ...
  if ($totalResultSetSize > 0) {
    ...
    if (isset($result) && $result->numChanges > 0) {
      printf("Number of ad units assigned to the placeement: %d\n",
          $result->numChanges);
    } else {
      printf("No ad units were assigned to the placement.\n");
    }
  }
