...
class DfpSessionBuilderTest extends TestCase
{
    ...
    public function testBuildFrom()
    {
        ...
        $dfpSession = $this->dfpSessionBuilder->from($configurationMock)
            ->withOAuth2Credential($this->fetchAuthTokenInterfaceMock)->build();
        $this->assertSame('12345678', $dfpSession->getNetworkCode());
        $this->assertSame(
            'google report runner',
            $dfpSession->getApplicationName()
        );
        ...
        $this->assertInstanceOf(
            StreamHandler::class,
            $dfpSession->getSoapLogger()->getHandlers()[0]
        );
        $this->assertSame(
            Logger::DEBUG,
    ...
    public function testBuildFromWithCustomLogger()
    {
        ...
        $dfpSession = $this->dfpSessionBuilder->from($configurationMock)
            ...
            ->withSoapLogger(new Logger('', [new NullHandler()]))->build();
    ...
    {
        $this->dfpSessionBuilder->withApplicationName('Google report runner')
            ->withOAuth2Credential($this->fetchAuthTokenInterfaceMock)->build();
    }
    ...
    {
        $dfpSession = $this->dfpSessionBuilder->withNetworkCode('12345678')
            ->withApplicationName('Google report runner')->withEndpoint(
                'https://abc.xyz/'
            )->withOAuth2Credential($this->fetchAuthTokenInterfaceMock)->build(
            );
    ...
    {
        $dfpSession = $this->dfpSessionBuilder->withNetworkCode('12345678')
            ->withApplicationName('Google report runner')->withOAuth2Credential(
                $this->fetchAuthTokenInterfaceMock
            )->build();

        $this->assertInstanceOf(
            LoggerInterface::class,
            $dfpSession->getSoapLogger()
        );
        $this->assertSame('12345678', $dfpSession->getNetworkCode());
        $this->assertSame(
            'Google report runner',
